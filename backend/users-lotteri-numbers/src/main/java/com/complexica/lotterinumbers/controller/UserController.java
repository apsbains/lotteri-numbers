...
public class UserController {
    ...
    @ResponseStatus(HttpStatus.CREATED)
    public UsersGeneratedNumbers generateNumber(@PathVariable String name) throws UserNotFoundException{
             
        return userLotteriService.generatedNumber(name);
    }

    @GetMapping("/users/{name}/numbers")
    public ResponseEntity<List<UsersGeneratedNumbers>> getUserNumbers(@PathVariable String name) throws UserNotFoundException{
